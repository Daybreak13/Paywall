...
namespace Paywall {
    ...
    public class VerticalAutoScroller : MonoBehaviour {
        [field: SerializeField] public bool Debug { get; protected set; }
        [field: SerializeField] public ScrollRect ScrollRectComponent { get; protected set; } 
        [field: SerializeField] public float ScrollPadding { get; protected set; } = 20f;

        protected GameObject _previous = null;

        protected virtual void Update() {
            DetectScroll();
        }

        protected virtual void DetectScroll() {
            GameObject current;
            Rect currentRect = new Rect();
            Rect viewRect = new Rect();
            RectTransform view = ScrollRectComponent.GetComponent<RectTransform>();
            ...
            if (current != null && current.transform.parent == transform) {
                ...
                if (current != _previous) {
                    ...
                    viewRect = GetRect(ScrollRectComponent.transform.position, view.rect, view.offsetMax);
                    ...
                    if (heading.y > 0f && !viewRect.Contains(currentRect.max)) {
                        ...
                        if (Debug) UnityEngine.Debug.LogFormat("Scroll up {0}", distance); // Decrease y value
                    }
                        ...
                        view.anchoredPosition = new Vector2(view.anchoredPosition.x, view.anchoredPosition.y + distance);
                        if (Debug) UnityEngine.Debug.LogFormat("Scroll down {0}", distance); // Increase y value
                    }
        ...
        public static void DrawBoundary(Rect rect, Color color) {
            ...
            UnityEngine.Debug.DrawLine(rect.min, topLeft, color); // Top
            UnityEngine.Debug.DrawLine(rect.max, topLeft, color); // Left
            UnityEngine.Debug.DrawLine(rect.min, bottomRight, color); // Bottom
            UnityEngine.Debug.DrawLine(rect.max, bottomRight, color); // Right
        }
    }
}