...
namespace Paywall {
    ...
    public class DamageOnTouch_PW : MonoBehaviour_PW {
		...
		[field: Header("EX Charge")]

		/// How much EX charge is gained per point of damage inflicted by this object
		[field: Tooltip("How much EX charge is gained per point of damage inflicted by this object")]
		[field: SerializeField] public float ChargeAmountGained { get; protected set; }
		...
		protected virtual void Awake() {
			...
			_health = this.gameObject.GetComponent<Health_PW>();
		...
		protected virtual void ApplyDamageCausedKnockback(float damage, List<TypedDamage> typedDamages) {
			...
			MovingRigidbody movingRigidbody = _colliderRigidbody2D.GetComponent<MovingRigidbody>();
			movingRigidbody.ApplyKnockback(DamageCausedKnockbackForce);
			...
			//		Vector2 relativePosition = _colliderRigidbody2D.transform.position - Owner.transform.position;
			//		_knockbackForce.x *= Mathf.Sign(relativePosition.x);
			//		break;
			//	case CausedKnockbackDirections.BasedOnSpeed:
			//		Vector2 totalVelocity = _colliderRigidbody2D.velocity + _velocity;
			...
			//		_knockbackForce.x *= Mathf.Sign((_colliderRigidbody2D.transform.position - (this.gameObject.transform.position + _colliderOffset)).x);
			//		break;
			//}
			...
			//		_colliderRigidbody2D.velocity = _knockbackForce;
			//		_characterJump = _colliderRigidbody2D.gameObject.MMGetComponentNoAlloc<Character>()?.FindAbility<CharacterJump>();
			...
			//		_colliderRigidbody2D.AddForce(_knockbackForce);
			//		_characterJump = _colliderRigidbody2D.gameObject.MMGetComponentNoAlloc<Character>()?.FindAbility<CharacterJump>();
			//		if (_characterJump != null) {
			//			_characterJump.SetCanJumpStop(false);
			//			_characterJump.SetJumpFlags();
		...
		protected virtual bool ShouldApplyKnockback(float damage, List<TypedDamage> typedDamages) {
			...
			return (_colliderRigidbody2D != null)
		...
		/// <summary>
		/// Apply knockback to self during self damage
		/// </summary>
		protected virtual void ApplyDamageTakenKnockback() {
			if ((_rigidbody2D != null) && (DamageTakenKnockbackForce != Vector2.zero) && (!_health.Invulnerable) && (!_health.PostDamageInvulnerable) && (!_health.ImmuneToKnockback)) {
				_knockbackForce.x = DamageCausedKnockbackForce.x;
				switch (DamageTakenKnockbackDirection) {
					case TakenKnockbackDirections.BasedOnSpeed: {
							Vector2 totalVelocity = _rigidbody2D.velocity + _velocity;
							_knockbackForce.x *= -1 * Mathf.Sign(totalVelocity.x);
							break;
						}
					case TakenKnockbackDirections.BasedOnDamagerPosition: {
							Vector2 relativePosition = _rigidbody2D.transform.position - _collidingCollider.bounds.center;
							_knockbackForce.x *= Mathf.Sign(relativePosition.x);
							break;
						}
				...
				switch (DamageTakenKnockbackType) {
					case KnockbackStyles.SetForce:
						_rigidbody2D.velocity = _knockbackForce;
						break;
					case KnockbackStyles.AddForce:
						_rigidbody2D.AddForce(_knockbackForce);
						break;
				}
			}
		}
